...
void loop() {
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (100.0 <= distance && distance <=200.0) {    // In desired Range
      light = 255.0 - (distance - 100.0)/100.0 * 255.0;
      analogWrite(PIN_LED, (int)light);             // LED ON      
  } else {
      light = (distance -200.0) /100.0 * 255.0;
      analogWrite(PIN_LED, (int)light);
  }
